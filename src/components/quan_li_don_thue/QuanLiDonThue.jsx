                        ...
                        {sessions.length === 0 ? (
                            ...
                            sessions.map((s) => (
                                <tr key={s.id}>
                                    ...
                                    <td>{s.ccdv.username}</td>
                                    <td>{s.address}</td>
                                    <td>{((new Date(s.endTime) - new Date(s.startTime)) / (1000 * 60 * 60)).toFixed(1)} giờ</td>
                                    <td>{formatDate(s.startTime)}</td>
                                    <td className="text-end">{formatMoney(s.totalPrice)}</td>
                                    <td className="text-center">
                                        <span className={`badge bg-${getStatusColor(s.status)}`}>
                                            {getStatusText(s.status)}
                                        </span>
                                    </td>
                                    <td className="text-center">{renderActionButton(s)}</td>
                                </tr>
                            ))
                        )}
                    </tbody>
